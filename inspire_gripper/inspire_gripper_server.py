...
class InspireGripperService(Node):

    def __init__(self):
        super().__init__('inspire_gripper_service')
        self.srv = self.create_service(SetBool, 'gripper_setting', self._gripper_setting_callback)
        self.gripper = InspireGripper()

    def _gripper_setting_callback(self, request, response):
        if(request.data == 0 ):
            self.gripper.two_finger_gripper(0)
            response.success = True
            response.message = 'Open the Gripper'
        elif(request.data == 1):
            self.gripper.two_finger_gripper(1)
            response.success = True
            response.message = 'Close the Gripper'
        else: 
            response.success = True
            response.message = 'Wrong Command for the Gripper!'
        return response
    
def main():
    rclpy.init()
    minimal_service = InspireGripperService()
